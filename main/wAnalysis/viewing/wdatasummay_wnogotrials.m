...
shoulder = 5; % avg 5 deg between pole positions , should = 2.5 deg
...
for(blk=1:numblocks)
    ...
    for(i=1:1:length(trialnums))
        ...
        if(frames<1500)
            ...
            past_meanbar= find((biased_bartheta+2*shoulder > thetaenv_bins) & (thetaenv_bins > biased_bartheta - shoulder));
            ...
            temp = thetaenv_pole((biased_bartheta+2*shoulder>thetaenv_pole) &  (thetaenv_pole> biased_bartheta- shoulder));
            if ~isempty(temp)
