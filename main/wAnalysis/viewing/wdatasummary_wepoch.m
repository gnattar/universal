...
for(blk=1:numblocks)
    ...
    var_set = {'meandev_thetaenv','peakdev_thetaenv','meanpole_thetaenv','prepole_thetaenv','prcoccupancy','prcoccupancy_epoch','meandev_whiskamp','totaldev_whiskamp','meanpole_whiskamp','totalpole_whiskamp'};
    col_set = {'k','r','b','g','k','k','k','b','k','b'};
    ...
    for v = 1: length(var_set)
        ...
        subplot(numblocks*2,1,(blk-1)*2+(1));title('Thetaenv Distribution from whisking epochs ');
        ...
        title('Thetaenv Distribution from sampling period ');
...
sc = get(0,'ScreenSize');
h1b = figure('position', [1000, sc(4)/10-100, sc(3)*3/10, sc(4)*3/4], 'color','w');
waterfall(occupancy_bins,trialnums,[1:size(thetaenv_occupancy,1)],thetaenv_occupancy); hold on;
