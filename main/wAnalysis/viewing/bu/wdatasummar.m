...
% w_setpoint_trials_width std of the points avgd over
    ...
    w_thetaenv = struct('trials',[],'time',[],'dist',[],'bins',[],'med',[],'medbinned',[],'meanbarcross',[],'meanbarcrossbinned',[],'prepole',[],'prepolebinned',[],'pval',[]);
...
%% plot theta
   ...
   numpoleframes = (restrictTime(2)-restrictTime(1))*500;
    ...
    for(blk=1:numblocks)
        ...
        thetaenv_meanbarcross = nan(length(trialnums),1);
...
%                     thetaenv_peak(i,1) = prctile(thetaenv_pole,85); %% change it to prctile later
                    thetaenv_meanbarcross(i,1) = prctile(thetaenv_pole,85);
