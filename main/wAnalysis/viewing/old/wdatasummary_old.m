...
function[w_setpoint_trials,w_setpoint_restricted,w_setpoint_trials_med,w_setpoint_trials_width,w_setpoint_trials_hist,pvalsetpoint...
    w_amp_trials,w_amp_restricted,w_amp_trials_med,w_amp_trials_width,w_amp_trials_hist,pvalamp]...
    =  wdatasummary(obj,block_tags,block_trialnums,avg_trials,gopix,nogopix,restrictTime,p,plot_whiskerfits,str,timewindowtag)
%% old version : compare accross blocks : changing to compare within block
% w_setpoint_trials % all setpoint values from all trials  entire length
% w_setpoint_restricted %just within the restricted time window
...
%    fpath = [p '/plots/' str];
    ...
    spbins = [-40:1:40];
    ampbins =[-5:.25:50];
    w_setpoint_restricted = cell(numblocks,1);
...
% % %          cbfreeze(colorbar);
        ...
        if(count-1<=avg_trials)
          ...
          if(blk==1)
                ...
                inds= [1:avg_trials] ;
          elseif(blk==numblocks)
              
                inds= [count-1-avg_trials : count-1] ;
          end
...
%         axis([-30 30 trialnums(1) trialnums(length(trialnums))])
          ...
          w_setpoint_restricted(blk) ={setpoint_trials(inds,round(restrictTime(1)/frameTime):round(restrictTime(2)/frameTime))};
...
%          cbfreeze(colorbar);
        axis([-1 30 trialnums(1) trialnums(length(trialnums))]);
          w_amp_restricted(blk) ={amp_trials(inds,round(restrictTime(1)/frameTime):round(restrictTime(2)/frameTime))};

%%plot setpoint last n trials
          avg_setpoint = mean(setpoint_trials(inds,:),1);
%          set(0,'DefaultAxesColorOrder',hot(count)); 
         xt=[1:size(setpoint_trials,2)]*frameTime;
         subplot(numblocks*2,4,(numblocks+blk-1)*4+1);plot(xt,setpoint_trials(inds,:),'color',[.5 .5 .5],'linewidth',1); hold on;           
         plot(xt,avg_setpoint,'linewidth',1,'color','k');hold off;
         axis([.3,4,-30,30]);set(gca,'YGrid','on');
         set(gca,'YTick',-30:10:30);
         freezeColors;

%%plot setpointhist last n trials         
         avg_setpointhist= mean(setpoint_trials_hist(inds,:),1);
         temp_avg_setpointhist(:,blk) = avg_setpointhist;
         subplot(numblocks*2,4,(numblocks+blk-1)*4+2);plot(spbins,setpoint_trials_hist(inds,:),'color',[.5 .5 .5],'linewidth',1); hold on;
         plot(spbins,avg_setpointhist,'linewidth',2,'color','k');hold off;
         axis([-40 40 0 max(max(setpoint_trials_hist(inds,:)))]);
         set(gca,'XTick',-40:10:40);set(gca,'XGrid','on');
         freezeColors;
         
  %%plot amp last 25 trials
          avg_amp = mean(amp_trials(inds,:),1);
%          set(0,'DefaultAxesColorOrder',hot(count)); 
         xt=[1:size(amp_trials,2)]*frameTime;
         subplot(numblocks*2,4,(numblocks+blk-1)*4+3);plot(xt,amp_trials(inds,:),'color',[.5 .5 .5],'linewidth',1); hold on;           
         plot(xt,avg_amp,'linewidth',1,'color','k');hold off;
         axis([.3,4,0,40]);set(gca,'YGrid','on');
         set(gca,'YTick',-30:10:30);
         freezeColors;

%%plot amphist last 25 trials         
         avg_amphist= mean(amp_trials_hist(inds,:),1);
         temp_avg_amphist(:,blk) = avg_amphist;
         subplot(numblocks*2,4,(numblocks+blk-1)*4+4);plot(ampbins,amp_trials_hist(inds,:),'color',[.5 .5 .5],'linewidth',1); hold on;
         plot(ampbins,avg_amphist,'linewidth',2,'color','k');hold off;
         axis([0 40 0 max(max(amp_trials_hist(inds,:)))]);
 ...
 % setpoint test for sig      
         temp1 = w_setpoint_restricted{blk}(1:100);temp1 = reshape(temp1,size(temp1,1)*size(temp1,2),1);
         temp2 = w_setpoint_restricted{blk}(end-100:end);temp2 = reshape(temp2,size(temp2,1)*size(temp2,2),1);
...
%         legend(block_tags);
 legend( 'first 100 trials','last 100 trials');
        fnam=[str 'avg_setpoint_hist'];  
        saveas(gcf,[fpath,filesep,fnam],'tif');
        saveas(gcf,[fpath,filesep,fnam],'fig');
        close(h1b);
%% amp test for sig        
         temp1 = w_amp_restricted{blk}(1:100);temp1 = reshape(temp1,size(temp1,1)*size(temp1,2),1);
         temp2 = w_amp_restricted{blk}(end-100:end);temp2 = reshape(temp2,size(temp2,1)*size(temp2,2),1);
