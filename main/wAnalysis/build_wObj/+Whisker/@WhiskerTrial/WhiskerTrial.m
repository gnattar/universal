classdef WhiskerTrial < handle
    ...
    methods (Access = public)
        function obj = WhiskerTrial(tracker_file_name, trial_num, trajectory_nums, varargin)
            ...
            try
                ...
                if exist([tracker_file_name '.bar'],'file')
                    ...
                    load('barposmat.mat');
                    list=dir('*.whiskers');
                    if(size(barposmat,1)== length(list))
                        thisfile=[tracker_file_name '.whiskers'];
                        trial=0;i=1;
                        while(trial==0)
                           trial= (strcmp(thisfile,list(i).name))*i;
                           i=i+1; 
                        end 
                        bar_f = (1:frame_nums(1))';
                        bar_x = repmat(barposmat(trial,1),frame_nums(1),1);
                        bar_y =repmat(barposmat(trial,2),frame_nums(1),1);  
                        obj.barPos = [bar_f, bar_x, bar_y];
                        obj.barPosClean = obj.barPos;%%GRchange
        ...
        function r = saveobj(obj)
            ...
            r.barPos = obj.barPos;
            r.barPosClean = obj.barPosClean; %%GRchange
