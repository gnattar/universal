...
%     temp =find(trialtypes==1);
%     hittrials = wSigtrials(temp);
%     temp=find(trialtypes==2);
%     misstrials =  wSigtrials(temp);
%     temp = find(trialtypes==4);
%     FAtrials = wSigtrials(temp);
end
pd =  pwd;
%% get trial blocks
...
% 
%         ind= find(ismember(hittrials,temp(1,:)));
%         blocks.hittrialnames{i}=hittrials(ind);
%         ind = find(ismember(misstrials,temp(1,:)));
%         blocks.misstrialnames{i} =  misstrials(ind);
% 
%         ind= find(ismember(CRtrials,temp(1,:)));
%         blocks.CRtrialnames{i}=CRtrials(ind);
%         ind = find(ismember(FAtrials,temp(1,:)));
%         blocks.FAtrialnames{i} =  FAtrials(ind);   

% 
%         nogotrialinds = zeros(length(blocks.nogotrialnames{i} ),1);
%         gotrialinds = zeros(length(blocks.gotrialnames{i}),1);
%         hittrialinds = zeros(length(blocks.hittrialnames{i} ),1);
%         misstrialinds = zeros(length(blocks.misstrialnames{i}),1);
%         CRrialinds = zeros(length(blocks.CRtrialnames{i} ),1);   
%         FAtrialinds = zeros(length(blocks.FAtrialnames{i}),1);
% 
% 
...
%         end
% 
%         for k=1:length(blocks.hittrialnames{i})
%             if (ismember(blocks.hittrialnames{i}(k),wSigtrials))
%                 hittrialinds(k)=find(wSigtrials==blocks.hittrialnames{i}(k));
%             end
%         end
% 
%         for k=1:length(blocks.misstrialnames{i})
%             if (ismember(blocks.misstrialnames{i}(k),wSigtrials))
%                 misstrialinds(k)=find(wSigtrials==blocks.misstrialnames{i}(k));
%             end
%         end
% 
%         for k=1:length(blocks.CRtrialnames{i})
%             if (ismember(blocks.CRtrialnames{i}(k),wSigtrials))
%                 CRtrialinds(k)=find(wSigtrials==blocks.CRtrialnames{i}(k));
%             end
%         end
% 
%         for k=1:length(blocks.FAtrialnames{i})
%             if (ismember(blocks.FAtrialnames{i}(k),wSigtrials))
%                 FAtrialinds(k)=find(wSigtrials==blocks.FAtrialnames{i}(k));
%             end
%         end
...
% 
%         hittrialinds(hittrialinds==0)=[];
%         misstrialinds(misstrialinds==0)=[];    
%         CRtrialinds(CRtrialinds==0)=[];
%         FAtrialinds(FAtrialinds==0)=[];    
% 
%         blocks.nogotrialnums{i}=nogotrialinds;
%         blocks.gotrialnums{i}=gotrialinds;
%         blocks.hittrialnums {i} = hittrialinds;
%         blocks.misstrialnums{i}=misstrialinds;
%         blocks.CRtrialnums{i}=CRtrialinds;
%         blocks.FAtrialnums{i}=FAtrialinds;
    end
end
