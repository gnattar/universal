...
for i = 1: nROI_effective
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            xt=CaSignal.CaTrials(1).FrameTime * [1:length(BG)];
            all_ind = [1:length(BG)];
...
%             if abs(F(i,xt_l) - nanmedian(BG(lighton_ind))) < abs(F(i,xt_l) - nanmedian(BG(lightoff_ind)))
%                 avg_BG(xt_l)=nanmedian(BG(lighton_ind));
%             else
%                 avg_BG(xt_l)=nanmedian(BG(lightoff_ind));
%             end
            
%             if abs(F(i,xt_h) - nanmedian(BG(lighton_ind))) < abs(F(i,xt_h) - nanmedian(BG(lightoff_ind)))
%                 avg_BG(xt_h)=nanmedian(BG(lighton_ind));
%             else
%                 avg_BG(xt_h)=nanmedian(BG(lightoff_ind));
%             end
    if get(handles.AnalysisModeBGsub,'Value') == 1
        if get(handles.lightstim_subtract,'Value') == 1
            F(i,:) = (F(i,:) -  avg_F)./sd_F;
