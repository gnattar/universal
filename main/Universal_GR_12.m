...
%      end      
    ...
    if get(handles.AnalysisModeBGsub,'Value') && get(handles.lightstim_subtract,'Value')
        if isempty(handles.lightstim_template)
            ...
            avg_F_lightoff = nanmedian(F(i,1:xt_l-2));
            avg_F_diff =  (avg_BG_lighton - avg_BG_lightoff); %* avg_F_lightoff/ avg_BG_lightoff;            
            avg_F(lighton_ind) = avg_F_diff;
            avg_F(lightoff_ind) = 0;          
            sd_BG_lighton = nanstd(BG(lighton_ind));
            sd_BG_lightoff = nanstd(BG(lightoff_ind));
            sd_F_lightoff = nanstd(F(i,1:xt_l-2));
            sd_F_diff =  (sd_BG_lighton / sd_BG_lightoff) ;% * sd_F_lightoff/ sd_BG_lightoff;
%               sd_F(lighton_ind) = sd_F_diff;
            sd_F(lighton_ind) = 1;
            sd_F(lightoff_ind) = 1; 
            F(i,:) = (F(i,:) -  avg_F)./sd_F;
%             F(i,xt_l-2:xt_l+2) = nan;
%             F(i,xt_h-2:xt_h+2) = nan;     
...
% %         %% if no ca event just scale the var
% %             top = mean(prctile(F(i,lighton_ind),70)) ;
% %             bottom = mean(prctile(F(i,lighton_ind),30)) ;
% %             bl_noise = 80; %% totally arbit
% %             if handles.lightstim_template(TrialName) && (abs(top - bottom) < (bl_noise *2)) %% no ca event
% %                 if nanstd(F(i,lighton_ind),[],2) > nanstd(F(i,lightoff_ind),[],2)*2
% %                      factor = nanstd(F(i,lighton_ind),[],2)/nanstd(F(i,lightoff_ind),[],2);
% %                      F(i,lighton_ind) = F(i,lighton_ind) / factor;
% %                 end
% %             end
            top = mean(prctile(F(i,lighton_ind),80)) ;
            bottom = mean(prctile(F(i,lighton_ind),30)) ;
            off = mean(prctile(F(i,lightoff_ind),30));
            F(i,lighton_ind) = F(i,lighton_ind) - handles.lightstim_template(TrialName) * (bottom - off);
...
% % %         
% % %         [N,X] = hist(F(i,lightoff_ind));
% % %         F_mode = X(find(N==max(N)));
% % % %         baseline_s(lightoff_ind) = mean(F_mode);
% % %         baseline_s(lightoff_ind) = median(F(i,lightoff_ind));
% % %         baseline_d = baseline_s;
% % %         baseline_d(:) = median(F(i,lightoff_ind));
% % %         dff(i,:) = (F(i,:)- baseline_s)./baseline_d*100;
% % %     else
        
        if get(handles.AnalysisModeDeltaFF,'Value')
%         [N,X] = hist(F(i,:));
%         F_mode = X(find(N==max(N)));
%         baseline = mean(F_mode);

        baseline = mean(prctile(F(i,:),20));
...
elseif(get(handles.sorted_CaTrials_select,'Value') ==1)
    ...
    if(get(handles.sortwtouchInfo,'Value')==1)
            ...
            count =0;
            if isfield(sorted_CaTrials,'lightstim')
                trialorder  = [intersect(sorted_CaTrials.lightstim ,sorted_CaTrials.touch ),intersect(sorted_CaTrials.nolightstim ,sorted_CaTrials.touch), intersect(sorted_CaTrials.lightstim ,sorted_CaTrials.notouch ),intersect(sorted_CaTrials.nolightstim ,sorted_CaTrials.notouch)];
                disp('order = touch notouch')
                trialtypes = zeros(length(trialorder),1);
                trialtypes(count+1:count+length(intersect(sorted_CaTrials.lightstim ,sorted_CaTrials.touch ))) = 1;
                count = count +length(intersect(sorted_CaTrials.lightstim ,sorted_CaTrials.touch ));
                trialtypes(count+1:count+length(intersect(sorted_CaTrials.nolightstim ,sorted_CaTrials.touch ))) = 2;
                count = count +length(intersect(sorted_CaTrials.nolightstim ,sorted_CaTrials.touch ));
            ...
            if isfield(sorted_CaTrials,'lightstim')
                ...
                for i=2:2:length(barpositions1)*2
                    inds= find(sorted_CaTrials.touch_barpos(t1)==barpositions1(round(i/2)))  ;  
                    touchinds(count+1:count+length(inds))=t1(inds);              
                    trialtypes(count+1:count+length(inds)) = i-1;
                    count = count +length(inds);
                    overlay(i-1) = 0;
                    
                    inds= find(sorted_CaTrials.touch_barpos(t2)==barpositions1(round(i/2)))  ;  
                ...
                offset = count;
                count=0;
                for j=2:2:length(barpositions2)*2
                    inds= find(sorted_CaTrials.notouch_barpos(t3)==barpositions2(round(j/2)))  ;  
                    ...
                    count = count +length(inds);
                    overlay(trialtypes_offset +j-1) = find(barpositions1==barpositions2(round(j/2)));
                    
                    inds= find(sorted_CaTrials.notouch_barpos(t4)==barpositions2(round(j/2)))  ;  
                    ...
                    count = count +length(inds);
                    overlay(trialtypes_offset +j) = find(barpositions1==barpositions2(round(j/2)));
                end
...
dur = 3.0;
wSigframerate = 500;
numpts=ceil((dur+baseline)*wSigframerate);% 3.5 seconds worth of data
