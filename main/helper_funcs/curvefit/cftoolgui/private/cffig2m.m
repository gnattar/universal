...
if isempty(down(dsdb)) && isempty(down(fitdb))
    errordlg( ...
        getString(message('curvefit:cftoolgui:CannotGenerateFileWhenNoDataSetsOrFitsExist')), ...
        getString(message('curvefit:cftoolgui:ErrorGeneratingFile')), ...
        'modal' );
    return
end
...
% preparation code
blks{end+1} = iComment( getString(message('curvefit:cftoolgui:SetUpFigureToReceiveDataSetsAndFits')) );
blks{end+1} = 'f_ = clf;';
blks{end+1} = 'figure(f_);';
fpos = cfgetfigurepos(cffig,'pixels');
blks{end+1} = sprintf('set(f_,''Units'',''Pixels'',''Position'',[%g %g %g %g]);',fpos);
if showlegend
    blks{end+1} = iComment( getString(message('curvefit:cftoolgui:LineHandlesAndTextForTheLegend')) );
    blks{end+1} = 'legh_ = [];';
    blks{end+1} = 'legt_ = {};';
end
blks{end+1} = iComment( getString(message('curvefit:cftoolgui:LimitsOfTheXaxis')) );
...
while ~isempty(ds)
    ...
    blkc{end+1} =  ' ';
    blkc{end+1} = iComment( getString(message('curvefit:cftoolgui:DataFromDataSet', dsname ) ));
    if ~isempty(xname)
        blkc{end+1} = iComment( sprintf( '    X = %s:',xname ) );
    end
    blkc{end+1} =  iComment( sprintf( '    Y = %s:', oldyname ) );
    if wtd
        blkc{end+1} = iComment( sprintf('    %s', getString(message('curvefit:cftoolgui:WeightsEqual', wname )) ));
    else
        blkc{end+1} = iComment( sprintf('    %s', getString(message('curvefit:cftoolgui:Unweighted')) ));
    end
    
    % Create code to plot this data set into the figure we have created
    blkd{end+1} = ' ';
    blkd{end+1} = iComment( sprintf('--- %s', getString(message('curvefit:cftoolgui:PlotDataThatWasOriginallyInDataSet',dsname))) );
    if isempty(xname)
    ...
    if ~isempty(dsline) && ishandle(dsline)
        ...
        blkd{end+1} = iComment(...
            getString(message('curvefit:cftoolgui:ThisDataSetDoesNotAppearOnThePlotToAddItToThePlotU'))  );
...
while ~isempty(ft)
    ...
    if ok
        ...
        blkf{end+1} = iComment( sprintf('--- %s',getString(message('curvefit:cftoolgui:CreateFit',fitname))) );
        ...
        if ~isequal(ft.outlier, cfGetNoneString() )
            ...
            blkf{end+1} = ' '; % blank line
            blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:ApplyExclusionRule',ft.outlier)) );
            ...
            if n>0
                ...
                blkf{end+1} = iErrorCode( ...
                    ...
                    getString(message('curvefit:cftoolgui:ExclusionRuleIsIncompatibleWith')), ...
                    ft.outlier, xname );
                blkf{end+1} = 'end';
        ...
        blkf{end+1} = iWarningCode( 'GenerateMFile:IgnoringNansAndInfs', ...
            getString(message('curvefit:cftoolgui:IgnoringNaNsAndInfsInData')) );
        blkf{end+1} = 'end';
        ...
        blkf{end+1} = ' '; % blank line
        blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:FitThisModelUsingNewData')) );
        if excluding
            % The generated code should throw an error too many points are
            % excluded.
            blkf{end+1} = 'if sum(~ex_(ok_))<2';
            blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:TooManyPointsExcluded')) );
            blkf{end+1} = iErrorCode( ...
                'GenerateMFile:NotEnoughDataAfterExclusionRule', ...
                 getString(message('curvefit:cftoolgui:NotEnoughDataLeftToFitAfterApplyingExclusionRule')), ...
                 fitname, ft.outlier );
            blkf{end+1} =  'else';
        ...
        if ~isequal(categ,'spline') && ~isequal(categ,'interpolant') && ~isempty(cvals)
            blkf{end+1} = iComment( sprintf( ...
                getString(message('curvefit:cftoolgui:AlternativelyUncommentTheFollowingLines')) ) );
        ...
        if ~isempty(ft.line) 
            ...
            blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:PlotThisFit')) );
            ...
            blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:TurnOffLegendCreatedByPlotMethod')) );
            blkf{end+1} = 'legend off;';
            if showlegend
                blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:StoreLineHandleAndFitNameForLegend')) );
            ...
            if isequal(ft.line.ShowBounds,'on')
                ...
                if showlegend
                    ...
                    blkf{end+1} = sprintf('   legt_{end+1} = ''%s'';', getString(message('curvefit:cftoolgui:PredBnds', quotedtext(fitname))));
                end
                blkf{end+1} = 'end';
            end
            ...
            if ~isempty(ft.rline) && ishandle(ft.rline)
                blkf{end+1} = ''; % blank line
                blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:ComputeAndPlotResiduals')) );
            ...
            blkf{end+1} = iComment( getString(message('curvefit:cftoolgui:ThisFitDoesNotAppearOnThePlot')) );
        end
    end
...
if haveresiduals
    blks{end+1} = iComment( getString(message('curvefit:cftoolgui:AxesForTheMainPlot')) );
    blks{end+1} = 'ax_ = axes;';
    blks{end+1} = 'set(ax_,''Units'',''normalized'',''OuterPosition'',[0 .5 1 .5]);';
    blks{end+1} = iComment( getString(message('curvefit:cftoolgui:AxesForTheResidualsPlot')) );
    blks{end+1} = 'ax2_ = axes;';
    blks{end+1} = 'set(ax2_,''Units'',''normalized'',''OuterPosition'',[0 0 1 .5]);';
    blks{end+1} = 'set(ax2_,''Box'',''on'');';
    if showlegend
        blks{end+1} = iComment( getString(message('curvefit:cftoolgui:LineHandlesAndTextForTheResidualsPlotLegend')) );
        blks{end+1} = 'legrh_ = [];';
        blks{end+1} = 'legrt_ = {};';
    end
else
    blks{end+1} = iComment( getString(message('curvefit:cftoolgui:AxesForThePlot')) );
    blks{end+1} = 'ax_ = axes;';
...
blkd{end+1} = iComment( getString(message('curvefit:cftoolgui:NudgeAxisLimitsBeyondDataLimits')) );
...
% Finish up
blke{end+1} = iComment( sprintf('--- %s', getString(message('curvefit:cftoolgui:FinishedFittingAndPlottingDataCleanUp'))) ) ;
blke{end+1} = 'hold off;';
if showlegend
    blke{end+1} = iComment( getString(message('curvefit:cftoolgui:DisplayLegend')) );
    ...
    if haveresiduals && nfit>0
         blke{end+1} = sprintf('leginfo_ = %s; %% %s',  ...
            cell2text(rleginfo), ...
            getString(message('curvefit:cftoolgui:PropertiesOfResidLegend')));
    ...
    blke{end+1} = iComment( getString(message('curvefit:cftoolgui:RemoveLabelsFromXAndYaxes')) );
    blke{end+1} = 'xlabel(ax_,'''');';
    blke{end+1} = 'ylabel(ax_,'''');';
    if haveresiduals
        blke{end+1} = 'xlabel(ax2_,'''');';
        blke{end+1} = 'ylabel(ax2_,'''');';
        blke{end+1} = iComment( getString(message('curvefit:cftoolgui:AddTitlesToThePlots')) );
...
blka{end+1} = iH1Comment( fcnname, getString(message('curvefit:cftoolgui:CreatesAPlotOfDataSetsAndFits')) );
blka{end+1} = iHelpComment( sprintf( '%s%s', upper( fcnname ), upper( argtext ) ) );
blka{end+1} = iHelpComment( getString(message('curvefit:cftoolgui:CreatesAPlotSimilar'))  );
blka{end+1} = iHelpComment( '' ); % blank comment line
blka{end+1} = iHelpComment( getString(message('curvefit:cftoolgui:NumberOfDataSets', nds )) );
blka{end+1} = iHelpComment( getString(message('curvefit:cftoolgui:NumberOfFits', nfit )) );

timestamp = iComment( getString(message('curvefit:cftoolgui:AutogeneratedByMATLABOn', datestr( now ) ) ));
...
if(fid<0)
    emsg = getString(message('curvefit:cftoolgui:CouldNotCreateFile', filename ));
    errordlg( emsg, getString(message('curvefit:cftoolgui:ErrorGeneratingFile')), 'modal' );
    return
end
...
% We will write to the editor if we have not been supplied with a file name
if ~filenameGiven
