...
try
    ...
    warning(ws);
    emsg = sprintf('%s\n%s', ...
        getString(message('curvefit:cftoolgui:ErrorColon')), ...
        e.message);
end
...
if isequal(analysis,'clear')
    ...
    text(.5,.5,getString(message('curvefit:cftoolgui:PressApplyToCreateANewPlot')),...
...
if isempty(fitobj)
    emsg = getString(message('curvefit:cftoolgui:CannotFindFit'));
...
try
    ...
    if ~isempty(x) && length(x) > 15000  %put up a waitbar
        hWaitbar = waitbar(0,getString(message('curvefit:cftoolgui:PleaseWait')), 'Name', getString(message('curvefit:cftoolgui:DataLoading')));
        waitbar(.05, hWaitbar);
    end
catch e
    emsg = sprintf('%s\n\n%s', ...
        getString(message('curvefit:cftoolgui:AttemptToEvaluateXiExpressionFailed')), ...
        e.message);
    emsg = combinewe(wmsg,emsg);
    return;
end
if isempty(x)
    emsg = getString(message('curvefit:cftoolgui:XiExpressionYieldedNoData'));
    emsg = combinewe(wmsg,emsg);
    return;
end
x = sort(x(:));
...
t1 = getString(message('curvefit:cftoolgui:Data'));
if ~isempty(hWaitbar) && ishandle(hWaitbar)
    waitbar(.25, hWaitbar);
end
level = 95;
if analysis.isInterpolateSelected && ~isempty(x)
    ...
    if bounds~=0
        ...
        try
            ...
            emsg = sprintf('%s\n\n%s', ...
                getString(message('curvefit:cftoolgui:AttemptToEvaluateLevelExpressionFailed')),...
                e.message);
            emsg = combinewe(wmsg,emsg);
            if ~isempty(hWaitbar) && ishandle(hWaitbar)
                close(hWaitbar)
        ...
        if ~isnumeric(level) || length(level)~=1 || level<=0 || level>=100
            emsg = getString(message('curvefit:cftoolgui:ConfidenceLevelMustBePositiveAndLessThan100'));
            emsg = combinewe(wmsg,emsg);
            if ~isempty(hWaitbar) && ishandle(hWaitbar)
                close(hWaitbar)
            end
    ...
    try
        ...
        switch bounds
            case 0
                ...
                t1 = getString(message('curvefit:cftoolgui:Fit'));
            case 1
                [ybnds,yfit] = predint(cfitobj,x,level/100,'functional');
                t1 = getString(message('curvefit:cftoolgui:FitWithPredBounds',levelStr));
        ...
        emsg = sprintf('%s\n%s', ...
            getString(message('curvefit:cftoolgui:ErrorEvaluatingFitErrorMessageIs')),...
            e.message);
        emsg = combinewe(wmsg,emsg);
        if ~isempty(hWaitbar) && ishandle(hWaitbar)
            close(hWaitbar)
...
try
    ...
    emsg = sprintf('%s\n%s', ...
        getString(message('curvefit:cftoolgui:ErrorComputingDerivativeErrorMessageIs')),...
        e.message);
    emsg = combinewe(wmsg,emsg);
    if ~isempty(hWaitbar) && ishandle(hWaitbar)
        close(hWaitbar)
...
if analysis.isIntegrateSelected
    if analysis.isStartMinXSelected
        ...
        try
            ...
            emsg = sprintf('%s\n\n%s', ...
                getString(message('curvefit:cftoolgui:AttemptToEvaluateStartFromExpressionFailed')), ...
                e.message);
            emsg = combinewe(wmsg,emsg);
            if ~isempty(hWaitbar) && ishandle(hWaitbar)
                close(hWaitbar)
        ...
        if ~isnumeric(x0) || length(x0)~=1 || isnan(x0) || ~isreal(x0)
            emsg = getString(message('curvefit:cftoolgui:StartingPointMustBeARealNumber'));
            emsg = combinewe(wmsg,emsg);
            if ~isempty(hWaitbar) && ishandle(hWaitbar)
                close(hWaitbar)
            end
    ...
    try
        ...
        x0Str = sprintf('%g', x0);
        t4 = getString(message('curvefit:cftoolgui:IntegralFrom',x0Str));
        results.integral = yint;
        results.integralstart = x0;
        wmsg = combineww(wmsg);
    catch e
        emsg = sprintf('%s\n%s', ...
            getString(message('curvefit:cftoolgui:ErrorComputingIntegralErrorMessageIs')),...
            e.message);
        emsg = combinewe(wmsg,emsg);
        if ~isempty(hWaitbar) && ishandle(hWaitbar)
            close(hWaitbar) 
...
if nplots > 0
    ...
    if isempty(plotfig) || ~ishandle(plotfig)
        plotfig=figure(...
            ...
            'Name', getString(message('curvefit:cftoolgui:CurveFittingAnalysis')),...
            'Numbertitle','off',...
    ...
    if plotpoints || ~isempty(yfit)
        ...
        if any(h~=0)
            ...
            c{1} = fitobj.name;
            levelStr = sprintf('%g', level);
            c{2} = getString(message('curvefit:cftoolgui:PredictionBounds',levelStr));
            c{3} = ds.name;
            c(h==0) = [];
            h(h==0) = [];
            legh = legend(h,c);
    ...
    txt = getString(message('curvefit:cftoolgui:AnalysisOfFitForDataset',...
        fitobj.name, ds.name));
    h = get(allax(1),'Title');
...
if ~isempty(w)
    e = sprintf('%s\n\n%s\n%s', ...
        e, ...
        getString(message('curvefit:cftoolgui:TheFollowingWarningAlsoOccurred')),...
        w);
end
...
if ~isempty(lastwarn)
    if isempty(w)
        w = sprintf('%s\n%s', ...
            getString(message('curvefit:cftoolgui:Warning')), ...
            lastwarn);
