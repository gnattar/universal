...
try
   ...
   uiwait(errordlg(getString(message('curvefit:cftoolgui:UnexpectedErrorOccurred', e.message )),...
                   getString(message('curvefit:cftoolgui:CurveFittingTool')),'modal'))
end
...
switch(action)
 ...
 case 'save'
   ...
   if nargin<2 || isempty(fn)
      [fn,pn] = uiputfile(filespec,getString(message('curvefit:cftoolgui:SaveSession')));
   ...
   try
      ...
      uiwait(errordlg(getString(message('curvefit:cftoolgui:ErrorSavingSessionFile', e.message )),...
                      getString(message('curvefit:cftoolgui:SaveError')),'modal'))
      ok = 0;
      return
   end
   ok = 1;
 ...
 case 'load'
   ...
   if nargin<2 || isempty(fn)
      [fn,pn] = uigetfile(filespec,getString(message('curvefit:cftoolgui:LoadSession')));
      if isequal(fn,0) || isequal(pn,0)
         return
      end
      if ~ismember('.',fn)
   ...
   try
      ...
      uiwait(errordlg(getString(message('curvefit:cftoolgui:ErrorLoadingSessionFile', e.message )),...
                      getString(message('curvefit:cftoolgui:LoadError')),'modal'))
   ...
   for j=1:nrequired
      if ~isfield(s,varnames{j})
         uiwait(errordlg(getString(message('curvefit:cftoolgui:NotAValidCurveFittingSessionFile')),...
                      getString(message('curvefit:cftoolgui:FileInvalid')),'modal'))
         return
      end
   end
   ...
   if ~ismember(s.version,allversions)
      uiwait(errordlg(getString(message('curvefit:cftoolgui:BadVersionNumberInCurveFittingSessionFile')),...
                      getString(message('curvefit:cftoolgui:InvalidVersion')),'modal'))
      return
   end
