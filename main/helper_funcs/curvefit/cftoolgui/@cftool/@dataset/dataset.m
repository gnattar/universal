...
if nargs<4 || isempty(dsname)
   ...
   if ~isequal(weight,NONE)
      dsname = getString(message('curvefit:cftoolgui:DataWithWeights',dsname,weight));
   end
end
...
errstring = '';
if nans
    errstring = getString(message('curvefit:cftoolgui:IgnoringNaNsInData'));
end
if infs
    inferr = getString(message('curvefit:cftoolgui:IgnoringInfsInData'));
...
if cplx
    cplxerr = getString(message('curvefit:cftoolgui:UsingOnlyTheRealComponentOfComplexData'));
    if isempty(errstring)
        errstring = cplxerr;
    else
        errstring = sprintf('%s\n%s', errstring, cplxerr);
...
if ~isempty(errstring)
     uiwait(msgbox(errstring, getString(message('curvefit:cftoolgui:ImportData')), 'warn', 'modal'));
end
...
function iRenameLine(h)
if ~isempty( h.line )
    set( h.line, 'DisplayName', h.name );
end
