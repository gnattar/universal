...
warndlg( sprintf('%s:\n''%s''\n%s:\n\n%s',...
    getString(message('SPLINES:resources:dlg_InvalidExpressionPart1')),get(editfield,'String'),...
    getString(message('SPLINES:resources:dlg_InvalidExpressionPart2')),laster ), ...
    getString(message('SPLINES:resources:dlg_InvalidExpression')),'modal')
...
function output = iEvaluateEditField(editfield,oldval,~)
% iEvaluateEditField   Evaluate the string at editfield if possible
%
% If it is possible to evalute the string then the value of that string is
...
if nargin>1
    if ischar(output)
        ...
        if ~all(isfinite(output))
            output = getString(message('SPLINES:resources:NoNaNsOrInfsPlease'));
            bad_edit(editfield,output)
            set(editfield,'String',oldval)
        end
        if nargin==3
    ...
    case 2 % combine both cubic and quintic smoothing spline.
        ...
        switch tag
            ...
            case {'minusleft','editleft','plusleft'}% we specified the smoothing par ...
                ...
                output = iEvaluateEditField(hand.params(2,1),shortstr(lineud.par,'',1),1);
                if ischar(output),
                    set(hand.figure,'Pointer','arrow')
                    drawnow
                    return
            ...
            case {'minusrite','editrite','plusrite'} % the tolerance was modified
                ...
                output = iEvaluateEditField(hand.params(2,2),lineud.tol(1),1);
                if ischar(output),
                    set(hand.figure,'Pointer','arrow'),
                    drawnow
                    return
...
output = iEvaluateEditField(hand.params(2,4),lineud.knots(V),1);
if ischar(output)
    return
else
    knot = output;
...
output = iEvaluateEditField(hand.params(2,3),x(V),1);
...
elseif get(hand.method,'Value')==4 && any(x==xx)
    ...
    if ~doy % restore edit field to its former value
        ...
        if get(hand.dataline,'UserData')
            ...
            output = iEvaluateEditField(hand.params(2,5),yold,1);
            if ischar(output), return
            else
                yy = output;
            end
