...
if pflag==1, %res vs y
   ...
   for II=1:REPS,
      hf=figure;
      hplot=plot(pyvar,GLMLAB_INFO_{18}(:,II),'k+');
...
elseif pflag==2, %res vs cov
   ...
   for JJ=find(COLS==1), %which covar
     ...
     for II=1:REPS, %rep
        hf=figure;
        hplot=plot(GLMLAB_INFO_{15}(:,JJ),GLMLAB_INFO_{18}(:,II),'k+');
...
elseif pflag==4,
   ...
   for II=1:REPS,
      ...
      hplot=plot(GLMLAB_INFO_{19},GLMLAB_INFO_{18}(:,II),'k+');
...
elseif pflag==5,  %See McCullagh and Nelder p~398--Constant information scales used
   ...
   for II=1:REPS,
      hf=figure;
      hplot=plot(XAXIS,GLMLAB_INFO_{18}(:,II),'k+');
      hxl=xlabel(xlabtext);
      ylabtext=[typer,' Residuals'];
      hyl=ylabel(ylabtext);
      title2text=[typer,' Residuals (',titledis,') vs '];
...
elseif pflag==6, %fv vs qe
   ...
   for II=1:REPS,
      hf=figure('menubar','none');
      hplot=plot(cdfnorm(GLMLAB_INFO_{18}(:,II)),GLMLAB_INFO_{19},'k+');
