function [pooled_contactCaTrials_locdep] = whisktheta_dependence_decoder(pooled_contactCaTrials_locdep,cond,str,train_test,thetabins,disc_func,src,plot_on,reg)
p = thetabins;
par = 'sigpeak';
num_runs = 1;
num_tests = 100;
...
%             th_all = cell2mat(cellfun(@(x) x.theta, pooled_contactCaTrials_locdep,'uniformoutput',0));
    ...
    if ~isempty(find(l_trials == 1))
            ...
            elseif train_test == 0
                ...
                w = waitbar(0, 'Start mani LDA runs  ...');
                for n = 1:num_runs
                    [dist_n,dist_err_n,hist_n,chist_n,mEr_n,fr_correct_n,pOL_n,p_n,numpred,mdl_list,warnings] =run_classify(train_resp,train_pos,train_tk,test_resp,test_pos,test_tk,train_test,disc_func,plot_on,src,mdl_list, num_tests,reg);
                    dist_all{n} = dist_n;
                    dist_err{n} = dist_err_n;
                    hist_all{n} =hist_n;
                    chist_all{n} =chist_n;
...
for s = 1:num_tests
    ...
    for t = 1:length(v)
        inds = find(ib==t);
        testID(count+1:count+numperclass,1) = inds(randperm(length(inds),numperclass));
        count = count+numperclass;
    end
    testsetsize = length(testID);
    testset{s} = testID;
end
...
parfor s = 1:num_tests
    ...
    test_tr = testset{s};
    if tt
        train_tr = setxor([1:test_numtrials],test_tr);
    elseif (strcmp(src,'NC') | strcmp(src,'NLS'))
        train_tr = setxor([1:train_numtrials],test_tr);
    else
        train_tr = randperm(train_numtrials,train_numtrials)';
    end
    
    S = test_resp(test_tr,:);
    Y = train_resp(train_tr,:);
    if tt
        %     class = classify(S,Y,train_pos(train_tr),disc_func);
        Mdl = fitcdiscr(Y,train_pos(train_tr),'Prior','uniform','DiscrimType',disc_func);
        if reg
            %regularization
            wm = warning('off','all');
    ...
    actual = test_pos(test_tr,1);
    cells_contrib = find(Mdl.DeltaPredictor>=Mdl.Delta);
    label = predict_gr(Y,train_pos(train_tr),S,cells_contrib);
    ...
    shuff_pos_train = train_pos(train_tr(shuff1));
    Mdl = fitcdiscr(Y(train_tr(shuff2)),shuff_pos_train,'Prior','uniform','DiscrimType',disc_func);
    %regularization
    %      [err,gamma,delta,numpred] = cvshrink(Mdl,'Gamma',1,'NumDelta',24,'Verbose',1);
    %     figure; plot(err,numpred,'k.');xlabel('Error rate');ylabel('Number of predictors');
    %     minerr = min(min(err))
...
% % % %     Mdl = fitcdiscr(Y,shuff_pos_train,'Prior','uniform','DiscrimType',disc_func);
% % % %      %regularization
...
% % % %
% % % % %       [v,a,vid] =unique(actual);
% % % % %       Mdl.Prior= sum(bsxfun(@eq,vid,[1:length(v)]))./length(vid);
% % % %
% % % %
% % % %     label = predict(Mdl,S);
% % % %     class = label;
