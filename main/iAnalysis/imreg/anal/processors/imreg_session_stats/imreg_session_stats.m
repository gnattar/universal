...
function retparams = process(params)
  ...
  % --- is this already done?  check for lockdown, and if done, nothing to do!
    ...
    master_target_im = mean(master_target_im,3);
		...
		save_image(master_target_im, [src_rootpath filesep 'master_imreg_image.tif'], 'hdr');
		save_image(session_mean_im, [src_rootpath filesep 'session_mean.tif'], 'hdr');
		save_image(session_sdmean_im, [src_rootpath filesep 'session_sdmean.tif'], 'hdr');
		save_image(session_sdmax_im, [src_rootpath filesep 'session_sdmax.tif'], 'hdr');
		save_image(session_maxproj_im, [src_rootpath filesep 'session_maxproj.tif'], 'hdr');
		save_image(session_sd_pertrial_im, [src_rootpath filesep 'session_pertrial_sd.tif'], 'hdr');
		save_image(session_mean_pertrial_im, [src_rootpath filesep 'session_pertrial_mean.tif'], 'hdr');
