...
%    batch_nfiles -- 1: execute on ALL files
%                    2: execute on FIRST file only (this is good for, e.g., 
%                       pan-session processors that do a whole directory).
%
...
function processors = get_processors(processor_name)
  ...
  % --------------------------------------------------------------------------
  % processor to generate summary data after you've run image registration on a session's data
	processors(i).type_id = i;
	processors(i).name = 'Summarize Session Registartion';
	processors(i).subpath = 'imreg_session_stats';
	processors(i).func_name = 'imreg_session_stats';
	processors(i).gui_present = 1;
	processors(i).image_output = 0;
	processors(i).batch_style = 4; % requires all stacks before next step bc it summarizes!
	processors(i).batch_nfiles = 2;
  ...
  % --------------------------------------------------------------------------
  % processor to cull channels from stack(s)
	processors(i).type_id = i;
	processors(i).name = 'Channel Stripper';
	processors(i).subpath = 'chanstrip';
	processors(i).func_name = 'chanstrip';
	processors(i).gui_present = 1;
	processors(i).image_output = 1;
	processors(i).batch_style = 2; 
	processors(i).batch_nfiles = 1;
		...
		for i=1:length(processors)
		  if (strcmp(processors(i).name,processor_name))
			  new_processors = processors(i);
				break;
		  elseif (strcmp(processors(i).subpath,processor_name))
			  new_processors = processors(i);
				break;
			end
    end
