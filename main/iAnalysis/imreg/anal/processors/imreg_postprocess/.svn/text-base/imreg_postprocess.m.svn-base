...
function retparams = process(params)
  ...
  % --- preparation
	...
	% read the displacement vector: src_path, E, dtheta_r, dx_r, dy_r
...
function retparams = process_batch (params)
  ...
  retparams = [];
	...
	% 2) call process()
  retparams(1).value = par_generate('imreg_postprocess','process',pr_params, params(5).value, params(6).value);
...
%
% constructs a params structure that can be passed to process based on a passed
% structure.  This should have the following fields:
%
%   init_mf_size: how big initial medfilt? -1 for off
%   adapt_correct_params: [alpha beta gbs] parameters for adaptive correction -
%                         see process above.  -1 for off.
%   final_mf_size: how big is the final median filter? -1 for off
%
function pr_params = construct_params_from_structure(params)
  p_struct = params(7).value;

  % - construct params for process()
  pr_params(1).value = params(2).value; % path of image file to process
  pr_params(2).value = params(3).value; % output path
  pr_params(3).value = p_struct.init_mf_size;
	pr_params(4).value = p_struct.adapt_correct_params;
  pr_params(5).value = p_struct.final_mf_size;
