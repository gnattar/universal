function results = decoderRF (x_trial,y_trial,type,names_x,names_y,featureList,neurons,N_shifts,shiftsPerFeature,optionsTree,weight)
%%
%--------------------------------------------------------------------------
%
...
for fIdx = 1:N_decodeFeat,
    ...
    for shIdx = 1:vecN,
        for nIdx = 1:N,
            if vecShift(shIdx) == 0
                aux_name =  sprintf('%s%.3d','Cell_',listN(nIdx));  % This is the convention, keep it like this when decoderRF is called
            elseif vecShift(shIdx) < 0  % Negative shifts
                aux_name =  sprintf('%s%.3d%s%d','Cell_',listN(nIdx),'_N_Shift_',abs(vecShift(shIdx)));  
            else                        % Postive shifts
                aux_name =  sprintf('%s%.3d%s%d','Cell_',listN(nIdx),'_P_Shift_',abs(vecShift(shIdx)));  
            end
    ...
    for fIdx = 1:N_decodeFeat,
        ...
        % Training set
            ...
            if strcmp(typeTree,'regression')
                ...
                aux_pred                              = str2num(cell2mat(predict(bagTree,testY)));  % Here we assume categories are numeric
            end
        ...
        for fIdx = 1:N_decodeFeat, 
            ...
            % Training set
                ...
                if strcmp(typeTree,'regression')
                    ...
                    aux_pred                              = str2num(cell2mat(predict (bagTree,testY)));   % Prediction for test trials (never seen by the trees before)                    
                end
                x_predict_trial(fIdx,testTrials{j},:) = reshape(aux_pred,N_time,[])';  
