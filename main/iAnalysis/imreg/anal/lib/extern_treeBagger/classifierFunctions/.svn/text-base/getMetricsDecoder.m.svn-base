function results = getMetricsDecoder (x,names,featureList,varTrialType,varPolePosition,minTrials,list_feat,results)
%%
%--------------------------------------------------------------------------
%
...
% featureList : name of the features that were decoded
% list_feat   : the pointers of features decoded to the full feature matrix 'x'
...
for i=1:N,
    ...
    %------------
    idx = list_feat(i);
    squaredError        = (x_train(:,idx) - results.x_predict(:,i)).^2;
    squaredErrorPSTH    = (results.psth.data.all(i,:) - results.psth.model.all(i,:)).^2;
    squaredSignal       = x_train(:,idx).^2;
    squaredPSTHSignal   = results.psth.data.all(i,:).^2;  
    squaredRelativePSTHData  = reshape((squeeze(x(idx,:,:)) - repmat(results.psth.data.all(i,:),N_trials,1)).^2,1,N_time*N_trials);
    squaredRelativePSTHModel = reshape((squeeze(results.x_predict_trial(i,:,:)) - repmat(results.psth.data.all(i,:),N_trials,1)).^2,1,N_time*N_trials);
    y_model  = results.x_predict(:,i);
    y_data   = x_train(:,idx);
    ...
    %------------
    squaredError        = (squeeze(x(idx,:,:)) - squeeze(results.x_predict_trial(i,:,:))).^2;   % Trial by Time
    squaredErrorPSTH    = (results.psth.data.all(i,:) - results.psth.model.all(i,:)).^2; % This is already the error by time
    squaredSignal       = squeeze(x(idx,:,:)).^2;
    squaredPSTHSignal   = results.psth.data.all(i,:).^2;  
    squaredRelativePSTHData  = (squeeze(x(idx,:,:)) - repmat(results.psth.data.all(i,:),N_trials,1)).^2;
    squaredRelativePSTHModel = (squeeze(results.x_predict_trial(i,:,:)) - repmat(results.psth.data.all(i,:),N_trials,1)).^2;
    y_model  = squeeze(results.x_predict_trial(i,:,:));
    y_data   = squeeze(x(idx,:,:));
    y_psth   = repmat(results.psth.data.all(i,:),N_trials,1);
   ...
   % Raw
    ...
    for j=1:4,
        if length(trialsType{j}) > minTrials
            y_2            = squeeze(x(idx,trialsType{j},:));
            ypred_2        = squeeze(results.x_predict_trial(i,trialsType{j},:));
    ...
    if ~isempty (varPolePosition)
        for j=1:N_cat,
            if length(trialsMultipleGo{j}) > minTrials
                y_2            = squeeze(x(idx,trialsMultipleGo{j},:));
                ypred_2        = squeeze(results.x_predict_trial(i,trialsMultipleGo{j},:));
