...
%--------------------------------------------------------------------------
%
% Goal : to create a feature indicating the position of the pole. It is
%        similar to createVariableTrialType.m
% ----

%--------------------------------------------------------------------------

% varTrialType:       the name of the variable that identifies the position
%                     of the pole. The positions can be numeric or categorical (but if categorical they will be renumbered). 
%                     0 is reserved for no-pole present. So if 0 is used as one position (i.e. No-go) 
%                     it will be replaced by -10
%varEvent:            can be none (''), in which case it starts from time 1 (for instance to have a time-block that indicates trial-type)
%separate:            indicates whether it creates  separable variables for the different positions 
%startEvent:          if you want to creat time-blocks indicating trial-types. (for instance pre-pole, post-pole, etc). 
%                     You can specifies from when (startEvent) until startEvent + duration
...
%fix the polePosition for the NoGo to be -10 instead of 0 (as it happens in Leo's data)
...
for i=1:N_cat,
    ...
    else % If it is non-numeric it will be asigned a number 1...N_cat
...
if (isempty(varEvent))   % Here duration has to be defined and >0 (I do not check for error so far..)
% It will start from startEvent (1 if not specified) to startEvent + duration
    ...
    if separate == 1
        if duration == -1
            ...
            % This assume that the varEvent is one when present, 0 otherwise
