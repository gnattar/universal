function [n_best P_best V_best C_best] = oopsi_deconv (sig,fr,thr,tau,minRate)
V.dt   = 1/fr;
V.tau  = tau;
P.gam  = (1-V.dt/V.tau);
T      = length(sig);
minSig = max([thr*mad(sig,1)*1.4826 0.10 3*std(sig(find(sig<0)))]);
...
% First round
[n_best P_best V_best C_best]=fast_oopsi_diego(sig,V,P);
...
%     minSig = mean(C_best (find(n_best>0.01 & n_best <= minNbest)))*X(2)+X(1);
    minSig = max([thr*P.sig 0.10 3*std(sig(find(sig<0)))]);
    % Estimation of tau
...
%         h = exp(-[0:filtLength-1]*V.dt/V.tau);
    ...
    P.lam = sum(q(find(q>1)))/T;
    % Second round
    [n_best P_best V_best C]=fast_oopsi_diego(sig,V,P);
    X     = [ones(length(sig),1) C_best]\sig;
