...
%   to all points using extern_warpImage, and indeed implemented therein.  Here, 
	...
	if ( ~piecewise)
      ...
      for f=1:nframes;
				...
				if (length(warp_parms) > 1) % 1 per frame
	     		im_c(:,:,f) = extern_warpImage(im_s(:,:,f), warp_parms(f).originalMarks, warp_parms(f).desiredMarks);
				else % ONE warpfield
	     		im_c(:,:,f) = extern_warpImage(im_s(:,:,f), warp_parms.originalMarks, warp_parms.desiredMarks);
				end
			end
		end
