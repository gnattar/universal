...
function [dx_r dy_r dtheta_r] = imreg_postproc(dx, dy, dtheta, err, opt)
  ...
  % 2) --- adaptive correction -- iff piecewise AND you have a valid error vec
	if (piecewise && length(opt.corr) == 3 && length(err) > 0)
...
function [dx_r dy_r] = correct_displacement_vectors(dx, dy, lE, alpha, beta, gbs, debug)
  ...
  % cutoff?
  skip_cutoff = 0.7; % if median error is below this, dx_r and dy_r are simply assigned dx and dy
	if (mE < skip_cutoff)
	  dx_r = dx;
		dy_r = dy;
		disp('imreg_postproc::skipping adaptive correction because of cutoff (see source).');
		return;
	end
