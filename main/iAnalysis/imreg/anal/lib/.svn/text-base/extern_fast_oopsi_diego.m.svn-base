function [n_best P_best V C_best]=extern_fast_oopsi_diego(F,V,P)
...
if V.fast_iter_max>1;
    ...
    if ~isfield(V,'fast_ignore_post'), V.fast_ignore_post=1; end % whether to ignore the posterior, and just keep the last iteration
end
...
if ~isfield(P,'lam'),   P.lam   = 0.5*V.dt*ones(V.Ncells,1);      end   %Modified by Diego
...
%% Parameter Update
    function P = est_params(n,C,F,P,b)
        ...
        if V.est_a==1 || V.est_b==1
            ...
            idx = find(n>0.05);     % DIEGO: Only calculate the regresion for responses not for noise
            if length(idx)<=10
                X2 = robustfit(A,F');
            else
                X2 = robustfit(A(idx),F(idx)');
            end
            X = A\F';
            P.a = X(2:end,:)';
            if V.est_b==1
            ...
            D   = F-X2(2)*(reshape(C,V.Ncells,V.T)) - b;  %DIEGO             
            mse = D(:)'*D(:);
        end
