	...
	for f=1:size(im,3)
	  ...
	  if (twofer)
		...
		% monochrome
			...
			%NORMAL: imshow(im(:,:,f), 'Border', 'tight');
			imshow(im(:,:,f), [0 .5], 'Border', 'tight');
			text(20,20,num2str(f*(1/4.07), '%.2f s'), 'Color', [1 1 1], 'FontWeight', 'bold', 'FontSize', 30); % this should be a passed param
%			text(20,20,num2str(f*2, '%.2f um'), 'Color', [1 1 1], 'FontWeight', 'bold', 'FontSize', 30); % this should be a passed param
			if (f >= 5 & f <=8) 
%			  text(200,20,'Pole in reach', 'Color', [1 0 0], 'FontWeight', 'bold', 'FontSize', 30); % this should be a passed param
			elseif(f >= 12 & f <=15) 
%			  text(200,20,'Answer period', 'Color', [0 1 1], 'FontWeight', 'bold', 'FontSize', 30); % this should be a passed param
			end
		end
