function [pixel_list,nuc_list,boundary_inner,boundary_outer]=extern_donut_roi(map,center,cell_radius,expand_factor,plot_flag)

%% EXTERN_DONUT_ROI.m  Select donut shaped ROI around a selected point
%    
%% Input Arguments
...
%
full_radius=1.3*cell_radius;
thick_inner=cell_radius*0.4;
thick_outter=cell_radius*0.05;
search_range=cell_radius*0.3; %maximally allowed half cell thickness in pixel
ntheta=90;
nsample=50;
...
%         rout_int=interp1(t,rout,theta);
        ...
        if (indx>0) && (indy>0) && indx<=(ss(2))  && indy<=(ss(1))
            if rr<rin_int
                roimap(round(indy),round(indx))=1;
            elseif (rr>=rin_int)  && (rr<rout_int)
                roimap(round(indy),round(indx))=2;
            end
        end
    end
end
