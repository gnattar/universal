...
% Last Modified by GUIDE v2.5 13-May-2010 21:43:59
...
glovars.fluo_control_main.display_mode_gauss_radio = handles.display_mode_gauss_radio;
glovars.fluo_control_main.norm_gauss_size_edit = handles.norm_gauss_size_edit;
...
glovars.fluo_control_main.processor_enable_multidir_checkbox = handles.processor_enable_multidir_checkbox;
...
set(handles.display_mode_gauss_radio, 'Value', false);
set(handles.norm_gauss_size_edit,'String', '0.02');

% Set default number of channels
set(handles.nchan_edit, 'String', num2str(glovars.fluo_display.nchan));
set(handles.usedchan_edit, 'String', num2str(glovars.fluo_display.usedchan));
...
% misc
glovars.fluo_control_main.parmat_warning = 0; % set to 1 once you've warned - allow exec
glovars.fluo_control_main.source_image_files = {}; % populate with source image file names if gotten from list
...
% handles    structure with handles and user data (see GUIDATA)
  ...
  % progress ; single call means 1 1
	params(4).value = [1 1];
...
% handles    structure with handles and user data (see GUIDATA)
  ...
  % --- multi-directory wildcard?
	if (get(glovars.fluo_control_main.processor_enable_multidir_checkbox,'Value'))
	  dir_list = get_fluo_dirs_from_wc(source_path);
		if (length(dir_list) < 1) ; disp('No matches found.') ; return ; end
		quest_txt{1} = 'The following directories will be processed: ';
		quest_txt(2:1+length(dir_list)) = dir_list(:);
		quest_txt{2+length(dir_list)} = 'Are you sure you want to do this?';
	  choice = questdlg(quest_txt, 'Warning of imminent death', ...
    ...
    if (proceed)
		  for d=1:length(dir_list)
			  ...
			  % build strings
			  source_path = dir_list{d};
				output_path = [dir_list{d} filesep 'fluo_batch_out' filesep];

				glovars.fluo_control_main.parmat_warning = 1; % disable warning flag
				disp('=============================================================================');
				disp(['Source path: ' source_path]);
				disp(['Output path: ' output_path]);
				disp(' ');
				run_batch_seq(source_path, output_path, source_wildcard);
			end
		end
...
% --- Executes batch sequence processing - either parallel or regular
%
function run_batch_seq(source_path, output_path, source_wildcard)
  global glovars;
  ...
  % get file list ; assign last_out_path for each file stream as the main source file
	...
	if (strcmp(source_wildcard, 'MANUAL')) % list in memory
	  source_files = glovars.fluo_control_main.source_image_files;
	else % do dir
  ...
  % --- TOP LOOP --- go thru all the processors 
    ...
    % if this has batch_style of 3 or 4, assign dep file for *previous* step
		if(glovars.processor_step(step_idx).processor.batch_style == 3 &  ...
		   s > 2)
		  dep_filepath = [glovars.par_path filesep 'parfile_' ustr '_' num2str(s-1) '_*']; % tmp and mat should be included
		end
		if(glovars.processor_step(step_idx).processor.batch_style == 4 &  ...
		   s > 2)
		  dep_filepath = [glovars.par_path filesep 'parfile_' ustr '_' num2str(s-1) '_*']; % tmp and mat 
		end

    % --- SECOND LOOP ---go thru all the files
    for f=1:length(source_files)
		  % for processors that only need one run
			if(glovars.processor_step(step_idx).processor.batch_nfiles == 2 & f > 1)
			  break;
			end

			% waitbar
			waitbar(((s-2)*length(source_files)+f)/((ns-1)*length(source_files)), wb, ...
			  [step_name ' processing: ' strrep(source_files{f},'_','-') ' ...']);
			...
			if (glovars.processor_step(step_idx).processor.image_output == 1)
				params(3).value = [output_path filesep strrep(glovars.processor_step(step_idx).processor.name,' ', '_') '_' num2str(step_idx) '_' source_files{f}];
			else
			  params(3).value = -1;
			end
      ...
      params(4).value = [f length(source_files)];
			...
			if (parallel_exec == 1)
				...
				if (s == 2 & f == 1) % VERY FIRST so check for unfinished
				  par_list = dir([glovars.par_path filesep '*.mat']);
					if (length(par_list) > 1 & ~ glovars.fluo_control_main.parmat_warning )
					  disp(['There are .mat files in ' glovars.par_path ' implying incomplete execution.  Check, then hit execute again if you are sure its okay.']);
						glovars.fluo_control_main.parmat_warning = 1; % set to 1 once you've warned - allow exec
						close(wb);
						return;
					elseif (glovars.fluo_control_main.parmat_warning)
	  				glovars.fluo_control_main.parmat_warning = 0; % reset warning for next time
					end
				end
				mat_fpath = [glovars.par_path filesep 'parfile_' ustr '_' num2str(step_idx) '_' num2str(f) '.mat'];
        params(5).value = mat_fpath;
				...
				func = 'process_batch';
				retparams = eval([glovars.processor_step(s).processor.func_name '(func,params);']);
      else % --- NON parallel much more simple
				% make the call to process SINGLE -- actually executes
				func = 'process_single';
				retparams = eval([glovars.processor_step(s).processor.func_name '(func,params);']);
			end
					...
					if (glovars.processor_step(step_idx).processor.image_output == 1)
						...
						if (last_step_is_base == 0)
						  ...
						  if (strcmp(last_out_path{f}, [im_path filesep im_fname]) ~= 1) 
							  disp(['I should delete ' last_out_path{f} ', but will not']);
%								delete(last_out_path{f});
    ...
    % if this has batch_style of 1, assign dep file for *next* step -- for parallel processing
		if(glovars.processor_step(step_idx).processor.batch_style == 1)
		  dep_filepath = [glovars.par_path filesep 'parfile_' ustr '_' num2str(s) '_*'];
		end
		if(glovars.processor_step(step_idx).processor.batch_style == 4)
		  dep_filepath = [glovars.par_path filesep 'parfile_' ustr '_' num2str(s) '_*'];
		end
...
% handles    structure with handles and user data (see GUIDATA)
	...
	global glovars;
	sps = get(handles.processing_steps_list, 'Value');
  ...
  for s=2:length(glovars.processor_step)
	...
	% 3) update gui ... cps
	if (glovars.current_processor_step >= sps) ; glovars.current_processor_step = sps-1 ; end
	if (glovars.current_processor_step == 0) ; glovars.current_processor_step = 1 ; end
	fluo_control_main_update_processing_steps_list();
...
%        contents{get(hObject,'Value')} returns selected item from processing_steps_list
  global glovars;
  sps = get(hObject, 'Value'); % SELECTED processor - what run and delete will apply to
	o_cps = glovars.current_processor_step;
	cps = sps; % CURRENT processor - the one that is DISPLAYED (may differ from SELECTED if SELECTED not processed)
  ...
  % IF the selected step outputs images but has not been run, tell the user
	...
	% now go back to most recent step that has image output that exists (if needbe) -- for the SHOWN processor, not SELECTED
	...
	if (sps ~= cps) ; disp('Could not show processor you selected; nevertheless, removes etc. will apply to it.'); end
...
% handles    structure with handles and user data (see GUIDATA)
  ...
  sourcepath = uigetdir(startpath);
	...
	if(0 == length(get(glovars.fluo_control_main.output_images_path_edit, 'String')))
	  set(glovars.fluo_control_main.output_images_path_edit, 'String', [sourcepath filesep 'fluo_batch_out' filesep]);
	end
...
% --- Executes on button press in select_source_image_files_button.
function select_source_image_files_button_Callback(hObject, eventdata, handles)
% hObject    handle to select_source_image_files_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
  global glovars;

  % 1) has source path been set? if not abort
	source_path = get(glovars.fluo_control_main.source_images_path_edit,'String');
	source_wildcard = get(glovars.fluo_control_main.source_images_wildcard_edit,'String');
  if (0 == length(source_path)) 
	  disp('Source path not specified.');
	...
	if (strcmp(source_wildcard, 'MANUAL')) ; source_wildcard = '*' ; end
	if (strcmp(source_wildcard, '')) ; source_wildcard = '*' ; end

	% 2) give user ability to select
  fpath = [source_path filesep source_wildcard];
	disp(['Allowing you to select from ' fpath '.']);
  flist = dir(fpath);
	fstr = {flist.name};
	...
	if (v ~= 0) % IF they set valid files ...
		% 3) assign wildcard to MANUAL
		set(glovars.fluo_control_main.source_images_wildcard_edit,'String', 'MANUAL');

		% 4) store in glovars.
		glovars.fluo_control_main.source_image_files = {};
		for i=1:length(s)
			glovars.fluo_control_main.source_image_files{i} = fstr{s(i)}; % populate with source image file names if gotten from list
		end
	end
...
% --- Executes on button press in display_mode_gauss_radio.
function display_mode_gauss_radio_Callback(hObject, eventdata, handles)
% hObject    handle to display_mode_gauss_radio (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of display_mode_gauss_radio

% GUI upate
set(handles.display_mode_frame_radio, 'Value', false);
set(handles.display_mode_mean_radio, 'Value', false);
set(handles.display_mode_max_radio, 'Value', false);
set(handles.display_mode_gauss_radio, 'Value', true);

% norm gauss size
ngs = get(handles.norm_gauss_size_edit, 'String');
if (length(ngs) == 0)
  disp('fluo_control_main::setting default gaussian size of 0.02');
	set(handles.norm_gauss_size_edit,'String', '0.02');
end

% assign mode ; update
global glovars;
glovars.fluo_display.display_mode = 4;
fluo_display_update_display();



function norm_gauss_size_edit_Callback(hObject, eventdata, handles)
% hObject    handle to norm_gauss_size_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of norm_gauss_size_edit as text
%        str2double(get(hObject,'String')) returns contents of norm_gauss_size_edit as a double
...
% --- Executes during object creation, after setting all properties.
function norm_gauss_size_edit_CreateFcn(hObject, eventdata, handles)
% hObject    handle to norm_gauss_size_edit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
%        str2double(get(hObject,'String')) returns contents of nchan_edit as a double
  ...
  % If changed, reload image
	if (oldnchan ~= glovars.fluo_display.nchan)
		cps = glovars.current_processor_step;
		fluo_display_open_stack (glovars.processor_step(cps).im_path, ...
			glovars.processor_step(cps).im_fname, []);
	end
...
% --- Executes on button press in processor_enable_multidir_checkbox.
function processor_enable_multidir_checkbox_Callback(hObject, eventdata, handles)
% hObject    handle to processor_enable_multidir_checkbox (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of processor_enable_multidir_checkbox
