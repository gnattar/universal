...
rois_name_tag = '';
    ...
    % % %          if(tag_trialtypes ==1)         
                ...
                for k = 1:length(types)
                    ...
                    axis([0 ts(length(ts)) -100 500]);set(gca,'YMinorTick','on','YTick', -100:100:500);
                    vline([ .5 1 1.5 2 2.5],'k-');
                end
...
%                     plot([frametime:frametime:length(detected_avg)*frametime] ,detected_avg,'color',col(types(k),:),'linewidth',1.5);           
                   ...
                   axis([0 ts(length(ts)) -50 250]);set(gca,'YMinorTick','on','YTick', -50:50:250);xlabel('Time(s)'); ylabel('mean_dFF');
