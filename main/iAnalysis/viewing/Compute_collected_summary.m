...
for i = 1: size(collected_summary,2) 
    ...
    for j = 1:d
        ...
        for k = 1:numtrials
            ...
            if (detected(k,1))
                try 
                    fwhm(k) = findFWHM([1:size(temp_data,2)],temp_data(k,:))*.0333;
                catch
                    intarea(k) = nan;
                    peakamp(k) = nan;
                    fwhm(k) = nan;
                ...
                intarea(k) = nansum(temp_data(k,:))*.0333;
        ...
        pooled_contact_CaTrials{count}.intarea_L= [nanmean(intarea(lightstim==1));nanstd(intarea(lightstim==1),[],1)/sqrt(sum(detected(lightstim==1)))];        
        pooled_contact_CaTrials{count}.intarea_NL=[nanmean(intarea(lightstim==0));nanstd(intarea(lightstim==0),[],1)/sqrt(sum(detected(lightstim==0)))];        
        cs_X = [0:50:800];
...
%         pooled_contact_CaTrials{count}.intarea_diff=cs_X(cs_L == .5) - cs_X(cs_NL == .5);
        ...
        pooled_contact_CaTrials{count}.peakamp_L= [nanmean(peakamp(lightstim==1));nanstd(peakamp(lightstim==1),[],1)/sqrt(sum(detected(lightstim==1)))];        
        pooled_contact_CaTrials{count}.peakamp_NL= [nanmean(peakamp(lightstim==0));nanstd(peakamp(lightstim==0),[],1)/sqrt(sum(detected(lightstim==0)))];        
        cs_X = [0:50:600];
...
%         pooled_contact_CaTrials{count}.peakamp_diff=cs_X(cs_L == .5) - cs_X(cs_NL == .5);
        ...
        pooled_contact_CaTrials{count}.fwhm_L= [nanmean(fwhm(lightstim==1));nanstd(fwhm(lightstim==1),[],1)/sqrt(sum(detected(lightstim==1)))];        
        pooled_contact_CaTrials{count}.fwhm_NL= [nanmean(fwhm(lightstim==0));nanstd(fwhm(lightstim==0),[],1)/sqrt(sum(detected(lightstim==0)))]; 
        cs_X = [0:.25:5];
