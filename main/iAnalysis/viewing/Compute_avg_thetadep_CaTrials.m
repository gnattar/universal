function  [pooled_contactCaTrials_thetadep] = Compute_avg_thetadep_CaTrials(collected_data,collected_summary)
...
for i = 1: size(collected_summary,2) 
    ...
    try
        ...
        temp_contactdir = arrayfun(@(x) x.contactdir, collected_data{1,i},'uniformoutput',0)';
        temp_contacts = arrayfun(@(x) x.contacts, collected_data{1,i},'uniformoutput',0);
        temp_tws = arrayfun(@(x) x.ts_wsk, collected_data{1,i},'uniformoutput',0)';
        temp_touchSetpoint = arrayfun(@(x) x.Setpoint, collected_data{1,i},'uniformoutput',0)';
        temp_touchdeltaKappa = arrayfun(@(x) x.deltaKappa, collected_data{1,i},'uniformoutput',0)';
    end
    ...
    for j = 1:d
        ttmin=min(cell2mat(arrayfun(@(x) size(x.dff(j,:),2), collected_data{1,i},'uniformoutput',0)'));
        tt= size( arrayfun(@(x) x.dff(j,:), collected_data{1,i},'uniformoutput',0)',1);
        data = nan(tt,ttmin);
        data = cell2mat(arrayfun(@(x) x.dff(j,1:ttmin), collected_data{1,i},'uniformoutput',0)');
        ...
        temp_lightstim =cell2mat(arrayfun(@(x) x.lightstim, collected_data{1,i},'uniformoutput',0)');
...
%         end
        ...
        pooled_contactCaTrials_thetadep {count}.touchSetpoint = temp_touchSetpoint;
        pooled_contactCaTrials_thetadep {count}.touchdeltaKappa = temp_touchdeltaKappa;
        ...
        pooled_contactCaTrials_thetadep{count}.trialnum= temp_solotrial;
        pooled_contactCaTrials_thetadep{count}.contactdir= temp_contactdir;
        pooled_contactCaTrials_thetadep{count}.contacts= temp_contacts;
        pooled_contactCaTrials_thetadep{count}.timews=temp_tws;
