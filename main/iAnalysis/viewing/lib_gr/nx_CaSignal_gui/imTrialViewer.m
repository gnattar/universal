...
% Last Modified by GUIDE v2.5 18-Jul-2010 14:28:08
...
function plot_param = init_plot_param(imTrArr)
[f, ts] = imTrArr.get_f_array(1);
plot_param.f = f;
plot_param.ts = ts(1,:);
plot_param.totTrials = imTrArr.nTrials;
plot_param.nTrials = 20;
plot_param.indTr = 1:20;  % initialize trial index to be plotted
plot_param.roiNo = 1; %
...
spacing = 1/(plot_param.nTrials + 2); % n*x + 3.5x = 1, space between plottings;

for i = 1: plot_param.nTrials
    ...
    if ~isempty(imTrArr.SoloTrials)
        ...
        time_pole_in(i) = imTrArr.SoloTrials{trNo}.pinDescentOnsetTime + 0.4;
...
plot_param.indTr = 1:plot_param.nTrials;
...
if a > plot_param.totTrials
    return
end
if b > plot_param.totTrials
    b = plot_param.totTrials;
end
...
switch nTrialsID
    ...
    case 6
        plot_param.nTrials = plot_param.totTrials;
    case 7
        plot_param.nTrials = input('Please specify number of trials to plot: ');
end
if plot_param.nTrials > plot_param.totTrials
    plot_param.nTrials = plot_param.totTrials;
