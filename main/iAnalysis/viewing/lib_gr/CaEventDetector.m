function Out = CaEventDetector(trace, Threshold, DecayThresh,slope_span, slopeThresh)
% Out = CaEventDetector(trace, Threshold, DecayThresh, slopeThresh, slope_span)
% % Event Detection
...
%     criteria(4) = criteria(1)*criteria(3) + criteria(2)*criteria(3);
    if sum(criteria) == true
        ...
        while trace(t1)>Threshold2 || (slope(t1)> 0 && trace(t1)>trace(t1-1)) 
            ...
            if eventNum >1 && t1<= Out(eventNum-1, 2)
                break;
            end
%             if t1<=1
%                 break;
