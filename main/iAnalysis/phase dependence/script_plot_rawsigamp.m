...
for d =1:length(dends)
    ...
    for i = 1:5
        ...
        title(['D ' num2str(dends(d)) ' CellID ' num2str(pcopy{dends(d)}.CellID) ' Ph ' num2str(ph(i))]); 
        count = count+1;
    end
subplot(nr,nc+1,count);
...
sc = get(0,'ScreenSize');
h_fig1 = figure('position', [1000, sc(4)/10-100, sc(3)*2/3, sc(4)], 'color','w');
ph=unique(pcopy{1}.theta_binned_new);
count =1;
for d =1:length(dends)
    temp = pcopy{dends(d)}.sigpeak;
    for i = 1:4
