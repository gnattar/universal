...
%             p_NL2CIL(i,:,1) = paramCI(1,:);p_NL2CIU(i,:,1) = paramCI(2,:);
        ...
        if ~isempty(L_ind)
            ...
            protract =  find(pooled_contactCaTrials_locdep{n}.totalKappa_epoch(L_ind)<0);
             ...
             plot(abs(pooled_contactCaTrials_locdep{n}.totalKappa_epoch(L_ind(retract))),pooled_contactCaTrials_locdep{n}.(par)(L_ind(retract)),'o','color',[.85 .5 .5],'Markersize',7,'Markerfacecolor',[.85 .5 .5]); hold on;
             plot(abs(pooled_contactCaTrials_locdep{n}.totalKappa_epoch(L_ind(protract))),pooled_contactCaTrials_locdep{n}.(par)(L_ind(protract)),'ro','Markersize',7, 'Markerfacecolor',[1 0 0]); hold on;
...
for d=1:length(dends)
    ...
    colrL = othercolor('BuGn4',10);
    colpL = othercolor('RdPu5',10);
    for i = 1:numloc
