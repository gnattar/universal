function [pooled_contactCaTrials_locdep] = whiskloc_dependence_plotdata(pooled_contactCaTrials_locdep,dends,par)
sc = get(0,'ScreenSize');
...
%         p_NL2CIL(i,:,2) = paramCI(1,:);p_NL2CIU(i,:,2) = paramCI(2,:);
        ...
        if ~isempty(L_ind)
            ...
            fittype = 'lin';
            [param,fitevals,f] = FitEval(x,y,fittype);
            temp (:,1) = x;temp(:,2) = y;temp(:,3) = f;
            pooled_contactCaTrials_locdep_fit.(['Fit_waves' str ]){i} = temp;
            pooled_contactCaTrials_locdep_fit.(['Fit_' par str 'fitparam'])(i,:,1) = param;
            pooled_contactCaTrials_locdep_fit.(['Fit_' par str 'fitevals'])(i,:,1) = fitevals;
            p_L1(i,:,1) = param;
...
%             plot([0.0001:.01:1.5],fsigm(pSig_L(i,:,2),[0.0001:.01:1.5]),'color',[.85 .5 .85],'linewidth',2);
    ...
    h= plot([1:numloc],p_NL1(:,1,1),'k-o'); set(h,'linewidth',1.5);hold on;
       plot([1:numloc],p_NL1CIU(:,1,1),'k--'); plot([1:numloc],p_NL1CIL(:,1,1),'k--');
    h= plot([1:numloc],p_NL1(:,1,2),'-o','color',[.5 .5 .5]); set(h,'linewidth',1.5);
        plot([1:numloc],p_NL1CIU(:,1,2),'--','color',[.5 .5 .5]); plot([1:numloc],p_NL1CIL(:,1,2),'--','color',[.5 .5 .5]);
    if ~isempty(L_ind)
        h= plot([1:numloc],p_L1(:,1,1),'r-o');set(h,'linewidth',1.5);hold on;
        h= plot([1:numloc],p_L1(:,1,2),'-o','color',[.85, .5 , .5]);set(h,'linewidth',1.5);
    end
    axis([0  numloc+1 -100  1000]);
