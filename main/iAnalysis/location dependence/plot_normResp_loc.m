...
ms_nl = round([mean(LPI_NL_list) std(LPI_L_list)]*100)./100;
ms_l=round([mean(LPI_L_list) std(LPI_L_list)]*100)./100;
...
tempnl(tempnl==0) = nan;
templ(templ==0) = nan;
subplot(1,2,2);hnl=hist(tempnl(:,1),bins);plot(bins,hnl,'k');hold on ;
if light
    hl=hist(templ(:,1),bins); plot(bins,hl,'r');
end
set(gca,'yscale','lin');
xlabel('norm Resp Amp at NonPref Loc','Fontsize',16);title('Hist norm Resp Amp at NonPref Loc','Fontsize',16);
set(gca,'Fontsize',16);
ms_nl = round([nanmean(LNPI_NL_list) nanstd(LNPI_L_list)]*100)./100;
ms_l=round([nanmean(LNPI_L_list) nanstd(LNPI_L_list)]*100)./100;
tb = text(2,10,[num2str(ms_nl(1)) '+/-' num2str(ms_nl(2))]);
tb = text(2,5,[num2str(ms_l(1)) '+/-' num2str(ms_l(2))]);set(tb,'color','r');
